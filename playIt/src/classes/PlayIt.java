...
public class PlayIt {
	 ...
	 * @return
	 */
	public static PlayIt getInstance(){
		if (instance == null) {
			instance = new PlayIt();	
		}return instance;
	}
	...
	public  List<String> getPerfilMusical(String idSessao) {
		...
		return Util.ordemTimeLine(retorno);
	}
	...
	*/
	...
	public List<String> getVisaoDosSons(String idSessao) throws Exception {
		...
		//lista com os usuarios que sao seus amigos
				...
				List<String> fontesDeSomOrdem = Util.ordemTimeLine(fontesDeSom);
				for (Usuario usuario : users) {
					for (int i = 0; i < fontesDeSomOrdem.size(); i++) {
						
						if (fontesDeSom.get(i).equals(usuario.getID())) {
							visaoDeSons.addAll(Util.ordemTimeLine(usuario.getPerfilmusical().getPerfil()));
						}
					}
				}
		...
		return Util.ordemTimeLine(visaoDeSons);*/
		//return visaoDeSons;
	}
	 ...
	 * @throws Exception 
	 */
	public List<String> getFontesDeSonsRecomendadas(String idSessao) throws Exception {
		if (Util.checaEhVazio(idSessao) || !sessoesAbertas.contains(idSessao)) {
			throw new SessaoInvalidaException();
		}
		...
		if(Util.usuarioSemFavoritosEFonteDeSons(userLogado)){
			//recomendar as fontes que mais têm sons favoritados
			//FALTA IMPLEMENTAR
		}else{
			for (Usuario user : users) {
				if ((!userLogado.getFonte().contains(user.getID())) || Util.usuarioSemFavoritosEFonteDeSons(user)) {//se o usuario ja nao esta na minha fonte de sons
			...
			List<Integer> maisRecomendados = Util.transformaSetEmList(recomendacao.keySet());
			System.out.println(recomendacao.keySet().toString()+ " mais recomendados");
			
			//int[] maisRecomendados = getMaisRecomendados(recomendacao.keySet());
			//List<Integer> maisRecomendados = Util.transformaSetEmList(recomendacao.keySet());
			Collections.sort(maisRecomendados);
			
			for (int i = 0; i < maisRecomendados.size(); i++) {
				String usuarioRecomendado = recomendacao.get(maisRecomendados.get(i));
				recomendados.add(usuarioRecomendado);
			}
			
			System.out.println("**********************LISTA DE RECOMENDADOS "+ recomendados.toString());
	...
	public List<String> getSonsEmLista(String idSessao, String nomeDaLista) throws Exception {
		...
		for (int i = 0; i < amigosEmLista.size(); i++) {
			...
			for(String som : Util.ordemTimeLine(perfil)){//coloca todos os sons do usuario na lista
				sons.add(som);
			}
		}
